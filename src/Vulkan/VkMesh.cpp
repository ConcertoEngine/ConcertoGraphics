...
{
	VkMesh::VkMesh(Vertices vertices, Allocator& allocator, std::size_t allocSize, VkBufferUsageFlags usage,
...
//		MapAndCopy(allocator, _vertexBuffer, _vertices.data(), allocSize);
	...
	VkMesh::VkMesh(const std::string& file, Allocator& allocator, VkBufferUsageFlags usage,
...
//		MapAndCopy(allocator, _vertexBuffer, _vertices.data(), _vertices.size() * sizeof(Vertex));
	...
	bool VkMesh::loadFromObj(const std::string& fileName, const std::string& materialPath)
	{
		std::string err;
			...
			Allocator& allocator)
	{
		if (!_isLoaded)
			throw std::runtime_error("Empty vertices");
		Buffer stagingBuffer(MakeAllocatedBuffer<Vertex>(allocator, _vertices.size() * sizeof(Vertex),
