...
namespace Concerto::Graphics::RHI
{
	VkRHIGpuSubMesh::VkRHIGpuSubMesh(RHI::SubMeshPtr meshPtr, RHI::MaterialPtr material, Vk::Allocator& allocator, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage) :
		...
		vertexBuffer(allocator, GetSubMesh()->GetVertices().size() * sizeof(Vertex), usage, memoryUsage)
	{
	...
	{
		Vk::Buffer stagingBuffer(MakeBuffer<Vertex>(allocator, GetSubMesh()->GetVertices().size() * sizeof(Vertex), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY));
