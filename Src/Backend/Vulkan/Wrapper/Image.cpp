...
namespace cct::gfx::vk
{
		...
		m_extent(extent)
	{
		...
		m_lastResult = vmaCreateImage(*device.GetAllocator().Get(), &imageCreateInfo, &imageAllocInfo, &m_handle, &m_allocation, nullptr);
	...
	Image::~Image()
	{
		...
		if (!m_isAllocated)
			return;
		vmaDestroyImage(*m_device->GetAllocator().Get(), m_handle, m_allocation);
	}
